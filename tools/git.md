...
`git branch <branch>` – Создаёт новую ветку (от текущего коммита), но не переключает на неё
...
`git branch -d <branch>` – Удаляет локальную ветку. Флаг -d безопасный: Git удалит ветку только если
её изменения уже слиты в другую ветку. Если ветка не слита, Git выдаст ошибку. Тогда можно
принудительно удалить: `git branch -D <branch>`

----------------------------------------------------------------------------------------------------

`git merge <branch>` – Сливает изменения из указанной ветки в текущую

`git rebase <branch>` – Переносит коммиты текущей ветки поверх указанной ветки (`git rebase -i HEAD~3` последние 3 коммита)

`git cherry-pick <commit>` – Применяет конкретный коммит из другой ветки в текущую ветку

- `git cherry-pick branch_name~2` - Берём предпоследний коммит из feature-branch
...
`git reset <commit>` - Перемещает текущую ветку на указанный коммит. В отличие от revert, reset
переписывает историю: коммиты после указанного могут исчезнуть из текущей ветки.
