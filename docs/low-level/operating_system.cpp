...
+-----------------------------------------------------------------------------------+
| - Это системное программное обеспечение, управляет абстракцией над аппаратными ресурсами (кроме cpu, mem),
|   другими аппаратными частями управляет драйвер
| - Слой между аппаратным обеспечением компьютера и прикладными программами
| - Предоставляет простой и удобный интерфейс для эффективного взаимодействия
|	с ресурсами системы для user_mode и прикладных программ
...
- прерывания, вектор прерывания, обработчики прерывания, аппаратные прерывания
- память, виртуальная память, страницы памяти
- системные вызовы
- режимы работы процессора и ОС
- планировщик в ос
- планирование исполнение задач, как на одном процессоре исполняются несколько программ, а где крутится в процессорном времени сама ос?
  нужно ли исполнение для самой ОС, или она -- просто реагирующая программа? ос нужен только для выполнения системных вызовах?
- таймеры в ос
- file, файловый дескриптор, файловый интерфейс, все - есть файл в unix
