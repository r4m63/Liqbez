...
# Администрирование и настройка на сервере

postgresql.conf
```
listen_addresses = '*' — Слушать входящие подключения на всех сетевых интерфейсах сервера.
port = 5432 — Порт, на котором работает сервер (поменяй на 9659, если нужно по заданию).
max_connections = 100 — Максимальное количество одновременных подключений к базе (стандарт).
superuser_reserved_connections = 3 — Резерв слотов для администратора, чтобы зайти при полной загрузке.

shared_buffers = 8GB — Объем оперативной памяти под кэш страниц базы данных (25% от RAM).
...
wal_level = replica — Уровень записи в WAL-журнал (необходим для репликации и бэкапов).
fsync = on — Принудительная запись данных на физический диск (гарантия сохранности при сбое питания).
synchronous_commit = on — Ждать физической записи WAL перед подтверждением транзакции (максимальная надежность).
...
random_page_cost = 1.1 — Стоимость случайного чтения. Для SSD ставим 1.1 (почти равно последовательному чтению 1.0).
effective_io_concurrency = 200 — Количество одновременных операций ввода-вывода. Для SSD ставим 200 (они умеют параллелить).

logging_collector = on — Включить перехват логов в файлы.
log_directory = 'pg_log' — Папка, куда будут падать файлы логов.
log_filename = 'postgresql-%a.log' — Имя файла лога (например, по дням недели).
...
log_checkpoints = on — Записывать в лог информацию о чекпоинтах (полезно для диагностики тормозов).
log_lock_waits = on — Записывать в лог сессии, которые долго ждут освобождения блокировки.

autovacuum = on — Включить автоматическую очистку мусора (обязательно).
...
samerole - пользователь может подключиться к базе, только если её имя совпадает с его именем (редкий кейс).
...
METHOD
trust - Пустить без вопросов. Пароль не нужен. Опасно для сети, нормально для local на машине разработчика.	❌ Низкая
reject - Отказать сразу. Полезно для бана конкретных IP.	🛡️ Высокая
scram-sha-256 - Стандарт. Самый безопасный метод передачи пароля (Challenge-Response). Пароль не летает по сети.	✅ Лучшая
md5 - Старый стандарт. Хранит хеш MD5. Используй только для совместимости с очень старыми клиентами.	⚠️ Средняя
password - Передает пароль открытым текстом. Использовать только внутри SSL-туннеля, иначе сниффер украдет пароль.	❌ Плохая
peer - Работает только для local. Берет имя пользователя операционной системы и пускает, если оно совпадает с именем пользователя БД. Пароль не нужен.	✅ Удобно для root/postgres
ident - То же, что peer, но для TCP. Спрашивает у Ident-сервера (порт 113) на машине клиента "кто это?". Устарело и небезопасно.	⚠️ Устарело
cert - Аутентификация по SSL-сертификату клиента. Пароль не нужен, нужен файл сертификата.	🛡️ Максимальная
ldap / radius / pam - Для интеграции с корпоративными системами (Active Directory и т.д.).	🏢 Enterprise
