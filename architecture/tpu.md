...
- Данные (например, изображения или тексты) загружаются в память CPU и подготавливаются (например, нормализуются (
  преобразование данных к единому масштабу (например, диапазону [0, 1] или стандартному распределению)) и
  батчируются(разделение исходного набора данных на небольшие группы (батчи или мини-батчи), которые последовательно
  подаются в модель для обучения или предсказания)).
...
3. Interconnect (Google TPU Pod(Это масштабируемый кластер из TPU, связанных через высокопроизводительную сеть)) –
   объединение TPU в кластер) - это Специализированная сеть для связи между TPU
...
Физическое соединение (PCIe, HBM(High Bandwidth Memory высокоскоростная память для GPU и ускорителей)) — это только
транспорт. Чтобы CPU мог передавать команды TPU, используются драйверы и
API.
...
- Activation Unit:
    - Этот блок отвечает за выполнение нелинейных операций, таких как функции активации (Функции активации – это
      математические фильтры, которые решают, какие данные дальше пойдут в нейросеть, а какие нет. Они делают так, чтобы
      модель могла понимать сложные зависимости, а не просто складывать числа.). В TPU за это отвечают Activation
      Units – маленькие ускорители внутри чипа, которые очень быстро считают эти функции. Они работают как светофор:
      ReLU – если число положительное, пропускаем его дальше, если отрицательное – заменяем на ноль.
      Sigmoid – сжимает любое число в диапазон от 0 до 1, удобно для вероятностей.
      Softmax – превращает кучу чисел в вероятности, например, что на фото кошка (90%), а не собака (10%).
      Tanh – как Sigmoid, но сжимает от -1 до 1, хорошо для баланса данных.
...
1. CPU (Central Processing Unit)
   ...
   ALU (Arithmetic Logic Unit) — выполняет арифметические и логические операции (сложение, умножение, сравнение).

   CU (Control Unit) — управляет потоком инструкций и данных.

   Особенности:

    - Универсальность: оптимизирован для последовательных задач (условия, ветвления, сложная логика).
    - Мало ядер (4–64), но каждое ядро мощное и автономное.
    - Кэш-память для уменьшения задержек.

   Пример задач: ОС, базы данных, веб-серверы.

2. GPU (Graphics Processing Unit)
   Вычислительные блоки:

   CUDA-ядра — тысячи упрощённых ALU, оптимизированных для параллельных вычислений.

   Tensor Cores (в современных GPU) — специализированные блоки для матричных операций.

   Особенности:

    - Массовый параллелизм: тысячи ядер для одновременной обработки данных (например, пикселей или вершин).
    - Нет сложного управления потоком — выполняет одну операцию на множестве данных (SIMD).
    - Высокая пропускная способность памяти (GDDR/HBM).

   Пример задач: Графика, машинное обучение (CNN), научные расчёты.

3. TPU (Tensor Processing Unit)
   Вычислительные блоки:

   MXU (Matrix Multiply Unit) — специализирован только для умножения матриц (основа ИИ-моделей).

   Vector Processing Unit (VPU) — вспомогательные операции (активации, нормализация).

   Особенности:

    - Жёстко заточен под тензорные операции (матрицы 16×16 или 128×128).
    - Нет универсальных ALU — только предопределённые ИИ-оптимизированные вычисления.
    - Минимальные задержки: данные и веса загружаются напрямую в MXU.

   Пример задач: Обучение/инференс нейросетей (Transformer, ResNet).
...
1. Специализация под ИИ

   Google создал Tensor Processing Unit (TPU) — первый в мире специализированный процессор для машинного обучения,
   оптимизированный исключительно под матричные операции (MXU). Это позволило:

   Ускорить обучение моделей в 10–100 раз по сравнению с GPU.

   Снизить энергопотребление за счёт отказа от универсальности.

2. Архитектурные инновации

   Matrix Multiply Unit (MXU) — жёстко заточен под умножение матриц (основа ИИ).

   High Bandwidth Memory (HBM) — решает проблему "бутылочного горлычка" памяти.

   Interconnect — масштабируемость до тысяч TPU в дата-центрах.

3. Влияние на индустрию

   Подтолкнул NVIDIA и AMD к созданию аналогичных решений (Tensor Cores, CDNA).

   Сделал ИИ доступнее — облачные TPU в Google Cloud снизили стоимость обучения моделей.

   Запустил гонку специализированных ускорителей (например, Cerebras, Graphcore).

4. Ограничения

   Не универсален — бесполезен вне задач ИИ.

   Закрытая экосистема — работает только в Google Cloud.
