...
**Идентификация** - процесс определения, кто пытается получить доступ. Обычно это ввод логина, email
или другого уникального идентификатора. Объявление о своей личности, без проверки на соответствие.

**Аутентификация** - проверка подлинности личности, то есть подтверждение, что вы действительно тот,
за кого себя выдаёте. Например, ввод пароля, биометрические данные или двухфакторная проверка.

**Авторизация** - определение прав и полномочий пользователя после успешной аутентификации.
Авторизация отвечает на вопрос: «Что этот пользователь может делать в системе?» Например, доступ к
определённым страницам, функциям или данным.
...
Механизм, при котором пользователь один раз проходит аутентификацию и потом получает доступ сразу к
нескольким связанным системам без повторного ввода логина и пароля.
