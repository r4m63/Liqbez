## 🧠 Основы программной инженерии — краткая выжимка
...
- Понятие возникло в **1960-х годах**.
- **Успешность мелких команд: ~70%**, крупных — только ~30%.
...
- **Разработка требований**  
  Определяются запросы заказчика и согласуются с разработчиками.  
  Четкие требования — основа успешной архитектуры и проектирования.

- **Анализ**  
  Постановка задач, выбор методов реализации и описание целей.

- **Проектирование**  
  Архитектура системы, шаблоны, структура компонентов.

- **Разработка**  
  Реализация по выбранной архитектуре, написание кода.

- **Тестирование**  
  Проверка на соответствие требованиям, отладка.

- **Внедрение**  
  Установка в рабочее окружение, подготовка к эксплуатации.

- **Эксплуатация**  
  Поддержка пользователей, устранение дефектов, обновление ПО.

- **Вывод из эксплуатации**  
  Завершение использования: переход на новую систему, миграция данных, удаление ПО.

---

### 🧠 Вывод:

Жизненный цикл ПО — это последовательность этапов, позволяющая управлять сложностью и качеством разработки. Каждый этап
требует четкой организации, планирования и взаимодействия между участниками проекта.

## 🔄 Группы процессов жизненного цикла ПО (ISO/IEC 12207:2010)

### 📌 Что такое процесс ЖЦ ПО?
...
### 🧩 Основные группы процессов (всего 43):

1. **Процессы согласования (2)**
    - Приобретение ПО
    - Поставка ПО

2. **Процессы организационного обеспечения (5)**
    - Менеджмент качества, рисков, ресурсов и инфраструктуры

3. **Процессы проекта (7)**
    - Планирование, оценка, управление проектами и конфигурациями

4. **Технические процессы (11)**
    - Анализ требований, архитектура, реализация, верификация, тестирование и приемка

5. **Процессы реализации программных средств (7)**
    - Проектирование, кодирование, тестирование компонентов

6. **Процессы поддержки ПО (8)**
    - Документация, управление конфигурацией, сопровождение, установка, помощь пользователям

7. **Процессы повторного использования ПО (3)**
    - Подготовка и управление повторно используемыми компонентами

---

### 📌 Особенности стандарта ISO/IEC 12207:

- Не описывает **конкретные действия**, а фиксирует **необходимость наличия процессов**.
- Каждый процесс описывается по шаблону:
- Входные данные и ресурсы → Совокупность действий → Выходные данные и ресурсы

- Входные/выходные данные могут включать:
- Требования по времени
- Бюджетные ограничения
- Спецификации и другие артефакты проекта

---

### 🧠 Вывод:

- **Процессы реализации кода — только малая часть** жизненного цикла.
- Для соответствия ISO стандарту необходимо покрытие всех 43 процессов.
- Следование структуре ISO повышает **качество разработки** и **надежность ПО**.

## 📊 Деление проектов по объему

### 📌 Категории проектов:

| Тип проекта    | Кол-во человек    | Продолжительность |
|----------------|-------------------|-------------------|
| **Малые**      | < 10 человек      | 3–6 месяцев       |
| **Средние**    | 20–30 человек     | 1–2 года          |
| **Большие**    | 100–300 человек   | 3–5 лет           |
| **Гигантские** | 1000–2000 человек | 7–10 лет          |

---

### 💸 Пример бюджета малого проекта:

- **Зарплата программиста:** 120 000 руб./мес
- **Общие трудозатраты:** 10 человек × 6 мес = ~7,2 млн руб.
- С учётом других затрат (офис, оборудование, налоги): **≈ 5–15 млн руб.**

---

### ⚠️ Проблемы с ростом объема:

- **Не линейный рост эффективности:**
    - Увеличение числа участников не увеличивает производительность пропорционально.
- **Возрастают накладные расходы:**
    - Администрация, бухгалтерия, продажи и т.д.
- **Падает согласованность:**
    - Больше участников → больше несогласованности, сложнее управлять.
- **Увеличивается бюрократия:**
    - Рост количества задач, документов, коммуникаций.
    - Это тормозит работу и снижает производительность.

---

### 🧠 Вывод:

- Увеличение масштаба проекта повышает затраты, но не обязательно — эффективность.
- Малые команды — дешевле и часто продуктивнее.

## 🧩 Модели жизненного цикла ПО (ЖЦП)

**Модель ЖЦП** — структура, определяющая порядок выполнения, взаимосвязь процессов и задач на всех этапах ЖЦ.

---

### 📌 Основные модели ЖЦП:

1. **Последовательная (водопадная / каскадная)**
    - Все требования определены заранее.
    - Этапы выполняются **однократно и строго по порядку**.
    - Плюсы: легко оценить сроки и стоимость.
    - Минусы: нет гибкости, поздняя обратная связь.

2. **Инкрементная**
    - Требования разбиты на части (инкременты).
    - ПО разрабатывается **по частям**, каждый инкремент — законченный функционал.
    - Основой для деления являются функциональные требования.

3. **Эволюционная**
    - **Не все требования известны изначально**.
    - Система развивается через **прототипы**.
    - Архитектура и функциональность уточняются в процессе.
    - SCRUM — пример эволюционного подхода.

4. **Формальных преобразований**
    - Используется редко.
    - Создается формальная модель ПО → на её основе генерируется код.
    - Преобразования выполняются по строгим математическим правилам.

---

### 🧠 Практика:

- На практике чаще применяют **инкрементно-эволюционные подходы**.
- Последовательная модель используется, когда:
    - требования стабильно известны;
    - нет необходимости в гибкости (пример: ПО для марсохода).

---

### 📷 Визуальные аналогии:

- Последовательная — дерево сразу растет целиком.
- Инкрементная — дерево растет по слоям.
- Эволюционная — дерево доращивается по мере уточнения требований.

## ⚙️ Методологии разработки ПО

### 🧩 Термины:

- **Метод** — последовательность шагов для достижения результата.
- **Методология** — наука, изучающая методы.
- В индустрии часто термин *"методология"* используется **не строго**, обозначая метод или модель.

---

### 🔄 Связь с ЖЦ ПО:

Методологии разработки базируются на моделях жизненного цикла ПО и определяют:
...
- Существует множество методологий, каждая из которых имеет плюсы и минусы.
- Важно понимать, **на какой модели ЖЦ они основаны**, и **что нового они вносят** в процесс разработки.

## 💧 Водопадная (каскадная) модель разработки ПО

### 📜 История:

- Разработана в **1960-х**.
- **Описана критически** Уинстоном Ройсом в **1970-х**.
- Часто используется как базовая модель жизненного цикла ПО.

---

### 📌 Суть модели:

- Последовательное прохождение фаз:
...
- Подходит для:
    - Проектов с **устойчивыми и понятными требованиями**
    - Жестких бюджетов и сроков (например, разработка ПО для миссий на Марс)
- Не подходит для гибкой, итеративной разработки.
...
этапы для повышения надежности.
...
Методология Ройса делает акцент на:

- Минимизации рисков через итерации и подготовку.
- Подтверждении реализуемости через прототипы.
- Тестировании как ключевом элементе успеха.
- Вовлечении пользователя в проверку и приёмку.

## ✅ Традиционная V-модель (V-chart model, 1977)
...
- Расширяет водопадную модель: каждому этапу разработки соответствует **этап верификации/тестирования**.
- Левая ветвь V — **разработка**, правая — **тестирование**.
- Тестирование идет **в обратном порядке к разработке**:
    - Требования → Приёмочное тестирование
    - Архитектура → Системное тестирование
    - Компоненты → Модульное тестирование
...
V-модель усиливает контроль качества за счёт тесной связи между стадиями проектирования и соответствующими этапами
тестирования. Используется там, где критична надежность и соответствие спецификациям (аэрокосмическая, военная сфера и
др.).
